...
| 探针 | 验证内容 | 用途 |
| --- | --- | --- |
| `/api/v1/health/live` | API 进程能够响应；不查询数据库或存储 | 判断进程存活 |
| `/api/v1/health/ready` | 数据库可查询且文件存储可写；未就绪返回 HTTP 503 | 发布切流与流量接入前检查 |
| `/api/v1/health` | 与 `/api/v1/health/ready` 使用同一处理逻辑 | 兼容现有 Compose 健康检查 |
| 静态站 `/healthz` | 对应 Nginx 静态入口可响应 | 分别检查学生端与管理端入口 |

发布时同时核验静态入口与 API 就绪状态。存活探针或静态站返回成功，不能替代数据库、文件存储和关键业务链路验证。
